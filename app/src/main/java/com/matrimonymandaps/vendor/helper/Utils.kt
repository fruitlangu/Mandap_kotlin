...
fun clearSavedCredentials(context: Context?): Boolean {
     PrefDelegate.init(context!!)
    var sharePrefStorage:SharePrefStorage= SharePrefStorage()

    val fcmToken: String? = sharePrefStorage.fireabseToken
    val loggedUserPhone: String? =sharePrefStorage.loggedUserPhone
    val latestAppVersionFromNotification: Int? = sharePrefStorage.updateVersion
    var enableNotificationStatus: Int? = sharePrefStorage.prefEnableNotification

    if (enableNotificationStatus === Constants.ENABLE_NOTIFICATION_LATER) {
        enableNotificationStatus = Constants.ENABLE_NOTIFICATION_NOT
    }
    clearPrefDelegate()
    ...
    sharePrefStorage.updateVersion=latestAppVersionFromNotification
    sharePrefStorage.prefEnableNotification=enableNotificationStatus

   showLog("clearSavedCredentials", "success ")
    return true
}
